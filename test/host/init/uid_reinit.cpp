...
static int init_uid(int rank, int nranks) {
    ...
    if (rank == 0) status = nvshmemx_get_uniqueid(&id);
    if (MPI_Bcast(&status, 1, MPI_INT, 0, MPI_COMM_WORLD) != MPI_SUCCESS || status) return 1;
    if (MPI_Bcast(&id, sizeof(id), MPI_UINT8_T, 0, MPI_COMM_WORLD) != MPI_SUCCESS) return 1;
