...
#define TEST_NVSHMEM_ALL_CUBIN()                                                                   \
    ...
    CU_CHECK(cuLaunchKernel(test_all_cubin, 1, 1, 1, 1, 1, 1, _dynamic_smem_size, cstrm, args_all, \
                            NULL));
...
#define TEST_NVSHMEM_RING_CUBIN()                                                                 \
    ...
    CU_CHECK(cuFuncSetAttribute(test_ring_cubin, CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, \
                                (int)_dynamic_smem_size));                                        \
    CU_CHECK(cuLaunchKernel(test_ring_cubin, 1, 1, 1, 1, 1, 1, _dynamic_smem_size, cstrm,         \
                            args_ring, NULL));
...
#define DEFINE_THREADGROUP_API(Group)                                                              \
    ...
    __global__ void ring_##Group(T *src, T *dest, size_t len, int nextpe,                          \
                                 size_t dynamic_smem_size) {                                       \
        rma_put_wrapper_##Group(src, dest, len, nextpe, dynamic_smem_size);                        \
                                                                                                   \
        nvshmem_quiet();                                                                           \
        NVSHMEM_TEST_RELEASE_SMEM(dynamic_smem_size);                                              \
    }                                                                                              \
    ...
    void launch_alltoall_##Group(void *src, void *dest, size_t len, int mype, int npes,            \
        ...
        if (use_cubin) {                                                                           \
            ...
            alltoall_##Group<T><<<1, 1, _dynamic_smem_size, cstrm>>>(src_, dest_, len, mype, npes, \
                                                                     _dynamic_smem_size);          \
        }                                                                                          \
    }                                                                                              \
                                                                                                   \
    ...
    void launch_ring_##Group(void *src, void *dest, size_t len, int nextpe, int prevpe,            \
        ...
        if (use_cubin) {                                                                           \
            ...
            ring_##Group<T><<<1, 1, _dynamic_smem_size, cstrm>>>(src_, dest_, len, nextpe,         \
                                                                 _dynamic_smem_size);              \
        }                                                                                          \
    }
