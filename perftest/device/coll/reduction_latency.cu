...
#define CALL_RDXN(TG_PRE, TG, TYPENAME, TYPE, OP, THREAD_COMP, ELEM_COMP)                          \
    ...
    void call_test_##TYPENAME##_##OP##_reduce_kern##TG##_cubin(                                    \
        ...
        NVSHMEM_PERF_CU_LAUNCH_COOP(test_##TYPENAME##_##OP##_reduce_kern##TG_cubin, num_blocks,    \
                                    num_tpb, stream, arglist, dynamic_smem_size);                  \
    }                                                                                              \
                                                                                                   \
...
#define CALL_RDXN_KERNEL(TYPENAME, OP, TG, BLOCKS, THREADS, ARG_LIST, STREAM)                     \
    if (use_cubin) {                                                                              \
        ...
        NVSHMEM_PERF_COLLECTIVE_LAUNCH(status, test_##TYPENAME##_##OP##_reduce_kern##TG, BLOCKS,  \
                                       THREADS, ARG_LIST, dynamic_smem_size, STREAM);             \
    }
