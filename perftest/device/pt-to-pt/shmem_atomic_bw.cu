...
int main(int argc, char *argv[]) {
    ...
    size_t atomic_bw_target_stride = 1;
#if defined(NVSHMEM_CFT_HANDLES_SUPPORT)
    if (use_smem) {
        atomic_bw_target_stride = 2;
    }
#endif
    ...
    void *args_skip[] = {(void *)&data_d,
                         (void *)&counter_d,
                         (void *)&(nelems),
                         (void *)&mype,
                         (void *)&skip,
                         (void *)&atomic_bw_target_stride,
                         (void *)&dynamic_smem_size};
    void *args_iter[] = {(void *)&data_d,
                         (void *)&counter_d,
                         (void *)&(nelems),
                         (void *)&mype,
                         (void *)&iter,
    ...
    if (mype == 0) {
        for (size = min_size; size <= max_size; size *= step_factor) {
            ...
            for (size_t repetition = 0; repetition < repetitions; repetition++) {
                ...
                h_msgrate[i] = calculate_msgrate(nelems, iter, milliseconds);
