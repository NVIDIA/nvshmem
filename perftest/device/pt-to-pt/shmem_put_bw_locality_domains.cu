...
static size_t put_messages_per_domain(size_t blocks, size_t threads) {
    switch (threadgroup_scope.type) {
        case NVSHMEM_THREAD:
...
int main(int argc, char *argv[]) {
    ...
    if (threadgroup_scope.type == NVSHMEM_WARP && threads_per_block % kWarpSize != 0) {
        fprintf(stderr,
                "shmem_put_bw_locality_domains: warp scope requires threads per CTA to be a "
                "multiple of %zu\n",
                kWarpSize);
        return EXIT_FAILURE;
    }
...

    {
        const size_t messages_per_domain =
            put_messages_per_domain(static_cast<size_t>(blocks_per_domain), threads_per_block);
