...
#error "shmem_putmem_signal_counted_ping_pong_latency requires NVSHMEM_CFT_HANDLES_SUPPORT"
#endif
...
__global__ void ping_pong(int *data_d, uint64_t *flag_d, uint64_t *ack_d, int len, int pe, int iter,
    ...
    for (int i = 0; i < iter; i++) {
        uint64_t expected = static_cast<uint64_t>(i + 1) * bytes;
        if (!pe) {
            int rc = nvshmemx_putmem_signal_counted_nbi_block(data_d, data_d, bytes, flag_d, peer);
            if (rc != NVSHMEMX_SUCCESS) {
                if (threadIdx.x == 0) {
                    *status = rc;
                    nvshmem_global_exit(rc);
                }
                return;
            }
            if (threadIdx.x == 0)
...
static void initialize_counted_case(int *data, uint64_t *flag, uint64_t *ack, int *status,
                                    size_t bytes, int mype) {
...
static int validate_counted_case(int *data, uint64_t *flag, uint64_t *ack, int *status,
                                 size_t bytes, int iterations, int mype) {
    int local_errors = 0;
    if (mype == 1) {
...
int main(int argc, char *argv[]) {
    ...
    if (use_cubin) {
        fprintf(stderr, "Counted ping-pong does not support cubin mode\n");
        finalize_wrapper();
        return EXIT_FAILURE;
    }
    ...
    constexpr size_t flag_allocation_size = kCountedCounterAlignment;
    ...
    if (use_mmap) {
        ...
        flag_d =
            (uint64_t *)allocate_mmap_buffer(flag_allocation_size, mem_handle_type, use_egm, true);
        DEBUG_PRINT("Allocated mmap buffer\n");
    } else {
        data_d = (int *)nvshmem_malloc(max_size);
        flag_d = (uint64_t *)nvshmem_align(kCountedCounterAlignment, flag_allocation_size);
    ...
    status_d = (int *)nvshmem_malloc(2 * sizeof(int));
    ack_d = (uint64_t *)nvshmem_malloc(sizeof(*ack_d));
    CUDA_CHECK(cudaMemset(ack_d, 0, sizeof(*ack_d)));
    CUDA_CHECK(cudaMemset(status_d, 0, 2 * sizeof(int)));
    ...
    peer = !mype;
    probe_counted_backend<<<1, threads_per_block>>>(data_d, flag_d, peer, status_d);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (size_t size = min_size; size <= max_size; size *= step_factor) {
        ...
        h_size_arr[i] = size;
        void *args_1[] = {&data_d, &flag_d, &ack_d, &nelems, &mype, &skip, &status_d};
        void *args_2[] = {&data_d, &flag_d, &ack_d, &nelems, &mype, &iter, &status_d};
        ...
        for (size_t repetition = 0; repetition < repetitions; repetition++) {
            ...
            if (validate_counted_case(data_d, flag_d, ack_d, status_d, size, iter, mype) != 0) {
                exit_code = 1;
                goto finalize;
            }
    ...
    if (mype == 0) {
        print_basic_table("shmem_putmem_signal_counted_ping_lat", "None", "latency", "us", '-',
                          h_size_arr, h_lat, i, h_lat_stats);
    }
finalize:
