...
def test_put_on_array(nvshmem_init_fini, dtype):
    ...
    def test_put(dst, src, pe):
        nvshmem.core.device.numba_cuda_mlir.put(dst, src, pe)
...
def test_get_on_array(nvshmem_init_fini, dtype):
    ...
    def test_get(dst, src, pe):
        nvshmem.core.device.numba_cuda_mlir.get(dst, src, pe)
...
def test_put_signal_on_array(nvshmem_init_fini, dtype):
    ...
    def test_put_signal(dst, src, signal_var, signal_val, signal_op, pe):
        nvshmem.core.device.numba_cuda_mlir.put_signal(dst, src, signal_var, signal_val, signal_op, pe)
...
def test_put_signal_with_wait_on_array(nvshmem_init_fini, dtype):
    ...
    def test_put_signal_with_wait(dst, src, signal_var, signal_val, signal_op, pe):
        nvshmem.core.device.numba_cuda_mlir.put_signal(dst, src, signal_var, 1, signal_op, pe)
        nvshmem.core.device.numba_cuda_mlir.signal_wait(signal_var, nvshmem.core.ComparisonType.CMP_GE, signal_val)
...
def test_signal_op_signal_wait():
    ...
    def test_signal_op_signal_wait(signal_var, signal_val, signal_op, pe):
        nvshmem.core.device.numba_cuda_mlir.signal_op(signal_var, 1, signal_op, pe)
        nvshmem.core.device.numba_cuda_mlir.signal_wait(signal_var, nvshmem.core.ComparisonType.CMP_GE, signal_val)
...
def test_p(dtype):
    ...
    def test_p(var, val, pe):
        nvshmem.core.device.numba_cuda_mlir.p(var, val, pe)
...
def test_g(dtype):
    ...
    def test_g(dest, var, pe):
        local = nvshmem.core.device.numba_cuda_mlir.g(var, pe)
        dest[:] = local
