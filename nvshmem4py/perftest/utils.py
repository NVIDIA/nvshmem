...
def run_coll_benchmark(args, coll):
    ...
    while n_elems <= max_elems:
        ...
        for _ in range(iters):
            ...
            time_ms = nvshmem.core.collective_on_buffer(coll,
                                                        nvshmem.core.Teams.TEAM_WORLD,
                                                        buf_dst,
                                                        buf_src,
                                                        dtype=dtype,
...
def print_runtime_options(args):
    ...
    print("min_size: {}, max_size: {}, step_factor: {}, iterations: {}, warmup iterations: {}, "
          "number of ctas: {}, threads per cta: {}, stride: {}, datatype: {}, reduce_op: {}, "
          "threadgroup_scope: {}, atomic_op: {}, dir: {}, report_msgrate: {}, bidirectional: {}, "
          "putget_issue: {}, use_graph: {}".format(args.min_size or 0, args.max_size or 0, args.step or 0, args.iters
                                                   or 0, args.warmup_iters or 0, args.ctas or 0, args.threads_per_cta
                                                   or 0, args.stride or 0, args.datatype or "None", args.reduce_op
                                                   or "None", args.scope or "None", args.atomic_op or "None", args.dir
                                                   or "None", int(args.msgrate), int(args.bidir), args.issue or "None",
                                                   int(args.cudagraph)))
    print("Note: Above is full list of options, any given test will use only a subset of these variables.\n")
    print(
        "Note: Python perftests do not yet reach full parity with C library perftests. Some options may be ignored.\n")
...
def build_parser():
    ...
    parser.add_argument("-t",
                        "--threads_per_cta",
                        type=int,
                        help="Number of threads per block (used in some device pt-to-pt tests)")

    parser.add_argument("-d",
                        "--datatype",
                        choices=[
                            "int", "int32", "uint32", "int64", "uint64", "long", "longlong", "ulonglong", "size",
                            "ptrdiff", "float", "double", "fp16", "bfloat16"
                        ],
                        default="float",
                        help="Datatype to use")

    parser.add_argument("-o", "--reduce_op", choices=["min", "max", "sum"], default="sum", help="Reduction operation")

    parser.add_argument("-s", "--scope", choices=["thread", "warp", "block", "all"], help="Scope of the operation")
    ...
    parser.add_argument("-a",
                        "--atomic_op",
                        choices=[
                            "inc", "add", "and", "or", "xor", "set", "swap", "fetch_inc", "fetch_add", "fetch_and",
                            "fetch_or", "fetch_xor", "compare_swap"
                        ],
    ...
    parser.add_argument("--dir", choices=["read", "write"], help="Whether to run put or get operations")

    parser.add_argument("--issue", choices=["on_stream", "host"], help="Applicable in some host pt-to-pt tests")
