...
/*
 * Host-side init functions paired with libnvshmem_device.a's device state.
 *
 * These live in the device archive alongside init_device.cu but are compiled
 * WITHOUT -rdc=true so that non-RDC user code can pull in nvshmemi_init_thread
 * (via the nvshmem_init() static inline) without dragging in RDC registration
 * symbols from init_device.o.
 */
...
/*
 * Explicit extern references to the __constant__ symbols defined in
 * init_device.cu.  We cannot rely on the EXTERN_CONSTANT header machinery
 * here: this TU is compiled non-RDC, so that path would emit a local per-TU
 * __constant__ copy, and cudaGetSymbolAddress below would then return the
 * address of the local copy instead of the archive-wide definition.
 */
...
#endif
#ifdef NVSHMEM_GPUNETIO_SUPPORT
extern __constant__ nvshmemi_gpunetio_device_state_t nvshmemi_gpunetio_device_state_d;
#endif
...
extern "C" int nvshmemi_get_device_state_ptrs(void **dev_state_ptr,
    ...
    switch (nvshmem_selected_device_transport) {
        ...
        case NVSHMEMI_DEVICE_TRANSPORT_TYPE_GPUNETIO_GDAKI:
            status =
                cudaGetSymbolAddress(transport_dev_state_ptr, nvshmemi_gpunetio_device_state_d);
            if (status) {
