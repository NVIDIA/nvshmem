...
__device__ NVSHMEMI_STATIC NVSHMEMI_DEVICE_ALWAYS_INLINE bool gdaki_can_coalesce_warp(
    ...
    if (amask != GDAKI_FULL_WARP) return false;
...
__device__ NVSHMEMI_STATIC NVSHMEMI_DEVICE_ALWAYS_INLINE void gdaki_wait_for_slot_availability(
    ...
    if (likely(wqe_idx >= nwqes)) {
        ...
        if (status) {
#ifdef NVSHMEM_GDAKI_DEBUG
            printf("gdaki_poll_cq failed with error=%d.\n", status);
#endif
        }
...
gdaki_quiet(nvshmemi_gpunetio_device_qp_t *qp) {
    ...
    if (prod_idx == 0) return 0;

#ifdef NVSHMEM_GDAKI_DEBUG
    printf("gdaki_quiet qp %x thread %d block %d wqe_idx %ld cqe_ci %ld\n", qp->qp.sq_num,
           threadIdx.x, blockDim.x, prod_idx, doca_gpu_dev_verbs_qp_get_cq_sq(&(qp->qp))->cqe_ci);
#endif

    int status = doca_gpu_dev_verbs_poll_cq_collapsed_at(&(qp->qp), prod_idx);
#ifdef NVSHMEM_GDAKI_DEBUG
    if (status != DOCA_SUCCESS)
        printf("doca_gpu_dev_verbs_poll_cq_collapsed_at failed %d.\n", status);
#endif
    assert(status == DOCA_SUCCESS);

#ifdef NVSHMEM_GDAKI_DEBUG
    printf("gdaki_quiet exit qp %x thread %d block %d wqe_idx %ld cqe_ci %ld\n", qp->qp.sq_num,
           threadIdx.x, blockDim.x, prod_idx, doca_gpu_dev_verbs_qp_get_cq_sq(&(qp->qp))->cqe_ci);
#endif
...
#ifdef NVSHMEM_GDAKI_DEBUG
            printf("rma_thread base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
    ...
    if (my_tid == 0) {
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("rma base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
...
__device__ NVSHMEMI_STATIC NVSHMEMI_DEVICE_ALWAYS_INLINE void nvshmemi_gdaki_rma_p_impl(
    ...
    if (my_tid == 0) {
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("rma_p_impl base_wqe_idx %ld num_wqe %d polling..\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
    ...
    if (can_coalesce_warp) gdaki_warp_sync();
...
__device__ NVSHMEMI_DEVICE_ALWAYS_INLINE T nvshmemi_gdaki_rma_g_impl(
    ...
    if (my_tid == 0) {
        ...
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("rma_g_impl base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
    ...
    if (my_tid == tg_size - 1) gdaki_release_ibuf(qp, base_ibuf_idx, num_ibuf_slots);
    ...
    if (my_tid == 0) {
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("amo_nonfetch_impl base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
...
nvshmemi_gdaki_amo_fetch_impl(void *rptr, const T value, const T compare, int pe, nvshmemi_amo_t op,
    ...
    if (my_tid == 0) {
        ...
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("amo_fetch_impl base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
    ...
    ret = READ_ONCE(*(T *)laddr);
    if (sizeof(T) == 4) ret = doca_gpu_dev_verbs_bswap32((uint32_t)ret);

    if (can_coalesce_warp) gdaki_warp_sync();

    if (my_tid == tg_size - 1) gdaki_release_ibuf(qp, base_ibuf_idx, tg_size);
    ...
    if (transfer_size == bytes) {
        ...
        if (my_tid == 0) {
            base_wqe_idx =
                doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                    &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
            printf("put_signal_thread1 base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
            gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
        }
        ...
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("put_signal_thread2 base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
...
__device__ NVSHMEMI_STATIC NVSHMEMI_DEVICE_ALWAYS_INLINE void nvshmemi_gdaki_put_signal_impl(
    ...
    if (my_tid == 0) qp = gdaki_get_qp(pe, qp_index);
    ...
    if (my_tid == 0) {
        base_wqe_idx =
            doca_gpu_dev_verbs_reserve_wq_slots<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
                &(qp->qp), num_wqes, DOCA_GPUNETIO_VERBS_GPU_CODE_OPT_SKIP_AVAILABILITY_CHECK);
#ifdef NVSHMEM_GDAKI_DEBUG
        printf("put_signal base_wqe_idx %ld num_wqe %d\n", base_wqe_idx, num_wqes);
#endif
        gdaki_wait_for_slot_availability(qp, base_wqe_idx + num_wqes);
    }
