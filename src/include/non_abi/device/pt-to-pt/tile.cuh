...
__device__ inline void nvshmemi_tile_cpy_nvl_threadgroup_vec(
    ...
    if constexpr (is_put) {
        ...
        assert((dst_v != nullptr) && "Failed to get nvlink ptr for "
        ...
        assert((src_v != nullptr) && "Failed to get nvlink ptr for "
...
__device__ inline void nvshmemi_tile_cpy_nvl_threadgroup_dim(src_tensor_t src_tensor,
    ...
    if (((size_t)src_tensor.data() % sizeof(int4) == 0) &&
        ...
        nvshmemi_tile_cpy_nvl_threadgroup_vec<int4, T, scope, tuple_t, is_put, major_dim, minor_dim>(
            ...
            start_coord, boundary, pe);
               ...
               (((get_stride_element<minor_dim>(dst_tensor) * sizeof(T)) % sizeof(uint64_t)) ==
                0)) {
        nvshmemi_tile_cpy_nvl_threadgroup_vec<uint64_t, T, scope, tuple_t, is_put, major_dim, minor_dim>(
            src_tensor.data(), dst_tensor.data(),
            get_shape_element<major_dim>(src_tensor),
            get_shape_element<minor_dim>(src_tensor),
            get_stride_element<minor_dim>(src_tensor),
        ...
        nvshmemi_tile_cpy_nvl_threadgroup_vec<uint32_t, T, scope, tuple_t, is_put, major_dim, minor_dim>(
            src_tensor.data(), dst_tensor.data(),
            get_shape_element<major_dim>(src_tensor),
            get_shape_element<minor_dim>(src_tensor),
            get_stride_element<minor_dim>(src_tensor),
...
template <typename src_tensor_t, typename dst_tensor_t, typename tuple_t, threadgroup_t scope, bool is_put>
__device__ inline void nvshmemi_tile_cpy_nvl_threadgroup(src_tensor_t src_tensor,
    ...
    if constexpr ((get_constant(safe_get<0>(decltype(src_tensor.stride()){})) == 1) &&
        ...
        nvshmemi_tile_cpy_nvl_threadgroup_dim<src_tensor_t, dst_tensor_t, tuple_t, scope, is_put, major_dim,
                                         minor_dim>(src_tensor, dst_tensor, start_coord,
                                                    boundary, pe);
...
template <typename src_tensor_t, typename dst_tensor_t, typename tuple_t, threadgroup_t scope,
          bool is_put, int major_dim, int minor_dim>
__device__ inline void nvshmemi_tile_cpy_put_get_threadgroup_wrapper(
    ...
    for (int i = myIdx; i < get_shape_element<minor_dim>(src_tensor); i += groupSize) {
        ...
        if constexpr (is_put) {
            ...
            nvshmemi_get_nbi_threadgroup<T, NVSHMEMI_THREADGROUP_THREAD>(
                dst_ptr, src_ptr, get_shape_element<major_dim>(src_tensor), pe);
        }
    }
}
...
__device__ inline void nvshmemi_tile_cpy_remote_threadgroup(src_tensor_t src_tensor,
    ...
    if constexpr ((get_constant(safe_get<0>(decltype(src_tensor.stride()){})) == 1) &&
        ...
        nvshmemi_tile_cpy_put_get_threadgroup_wrapper<src_tensor_t, dst_tensor_t, tuple_t, scope, is_put, major_dim,
                                         minor_dim>(src_tensor, dst_tensor, start_coord,
                                                    boundary, pe);
    } else if constexpr ((get_constant(safe_get<1>(decltype(src_tensor.stride()){})) == 1) &&
                         (get_constant(safe_get<1>(decltype(dst_tensor.stride()){})) == 1)) {
...
__device__ inline int nvshmemi_tile_put(src_tensor_t src_tensor,
    ...
    if constexpr (algo == nvshmemx::tile_algo_t::PEER_PUSH_NBI) {
        ...
        nvshmemi_tile_cpy_remote_threadgroup<src_tensor_t, dst_tensor_t, tuple_t, scope, 1/*is_put*/>(
            src_tensor, dst_tensor, start_coord, boundary, pe);
...
template <nvshmemx::tile_algo_t algo, typename src_tensor_t, typename dst_tensor_t,
          typename tuple_t, threadgroup_t scope>
__device__ inline int nvshmemi_tile_get(src_tensor_t src_tensor,
                                        dst_tensor_t dst_tensor, tuple_t start_coord,
                                        tuple_t boundary, int pe, uint64_t flag) {
#if defined(__cplusplus) && __cplusplus < 201703L
    assert(0 && "Tile-granular APIs need C++ 17");
