...
#include "non_abi/nvshmemi_error_macros.h"               // for NVSHMEMI_NZ_EXIT
...
static inline void nvshmemi_coll_p2p_sync(nvshmemi_team_t* teami, CUstream stream) {
    ...
    for (int i = 0; i < teami->size; i++) {
        if (i != teami->my_pe) {
            ...
            NVSHMEMI_CU_NZ_EXIT(nvshmemi_cuda_syms, status,
                                "nvshmemi_coll_p2p_sync cuStreamWriteValue64() failed\n");
        }
    }
    for (int i = 0; i < teami->size; i++) {
        if (i != teami->my_pe) {
            ...
            NVSHMEMI_CU_NZ_EXIT(nvshmemi_cuda_syms, status,
                                "nvshmemi_coll_p2p_sync cuStreamWaitValue64() failed\n");
        }
    }
}
