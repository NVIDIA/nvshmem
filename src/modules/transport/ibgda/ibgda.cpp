...
int ibgda_rc_progress(nvshmem_transport_t t) {
    ...
    int n_pes = t->n_pes;
    struct mlx5_wqe_ctrl_seg ctrl_seg;
    std::lock_guard<std::mutex> lock(ibgda_state->rc_endpoint_lock);
...
int nvshmemt_ibgda_get_mem_handle(nvshmem_mem_handle_t *mem_handle, void *buf, size_t length,
    ...
    if (transport->alias_va_map != NULL && transport->alias_va_map->count(buf)) {
        INFO(ibgda_state->common.log_level, "IBGDA: alias va found for buf: %p, alias va: %p", buf,
    ...
    for (int i = 0; i < n_devs_selected; ++i) {
        struct ibgda_device *device =
            ((struct ibgda_device *)ibgda_state->common.devices + ibgda_state->selected_dev_ids[i]);
        nvshmem_mem_handle_t *dev_handle = (nvshmem_mem_handle_t *)&handle->dev_mem_handles[i];

        INFO(ibgda_state->common.log_level, "[%d] IBGDA: device used %s, data_direct support: %d",
        ...
        status = nvshmemt_ib_common_reg_mem_handle(
            ...
            ibgda_state->dmabuf_support_for_data_buffers, ibgda_cuda_syms,
            ibgda_state->common.log_level, ibgda_state->common.options->IB_ENABLE_RELAXED_ORDERING,
            device->common_device.data_direct, alias_va_ptr);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
static void ibgda_destroy_cq_mobjects(struct ibgda_cq *gcq) {
    if (gcq->dbr_mobject) {
        if (gcq->dbr_mobject->has_nic_mapping) {
    ...
    if (gcq->cq_mobject) {
        if (gcq->cq_mobject->has_nic_mapping) {
...
static int ibgda_destroy_internal_buffer(struct ibgda_internal_buffer *internal_buf,
    ...
    if (internal_buf->mem_handle) {
        nvshmemt_ib_common_release_mem_handle(&ftable,
                                              (nvshmem_mem_handle_t *)internal_buf->mem_handle,
                                              ibgda_state->common.log_level);
...
static int ibgda_create_internal_buffer(struct ibgda_internal_buffer *internal_buf,
    ...
    status = nvshmemt_ib_common_reg_mem_handle(
        ...
        ibgda_cuda_syms, ibgda_state->common.log_level,
        ibgda_state->common.options->IB_ENABLE_RELAXED_ORDERING, device->common_device.data_direct);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
static void ibgda_destroy_qp_mobjects(struct ibgda_qp_control_structures *qp_ctrl) {
    ...
    if (qp_ctrl->prod_idx_cache) {
        free(qp_ctrl->prod_idx_cache);
    }

    if (qp_ctrl->prod_idx_snapshot) {
        free(qp_ctrl->prod_idx_snapshot);
    }

    if (qp_ctrl->dbr_mobject) {
        if (qp_ctrl->dbr_mobject->has_nic_mapping) {
            ibgda_mobject_nic_unmap(qp_ctrl->dbr_mobject);
        }
    ...
    if (qp_ctrl->wq_mobject) {
        if (qp_ctrl->wq_mobject->has_nic_mapping) {
            ibgda_mobject_nic_unmap(qp_ctrl->wq_mobject);
        }
...
static int ibgda_destroy_qp_shared_objects(struct ibgda_device *device) {
    int status = 0;
...
static int ibgda_create_qp_shared_objects(struct ibgda_device *device) {
    int status = 0;
...
out:
    if (status) {
        if (recv_cq) {
            ftable.destroy_cq(recv_cq);
        }
        if (srq) {
            ftable.destroy_srq(srq);
        }
    }
    return status;
}
...
static int ibgda_destroy_dct_shared_objects(struct ibgda_device *device) {
    int status = 0;
...
out:
    if (status) {
        if (ah) {
            ftable.destroy_ah(ah);
        }
        if (recv_cq) {
            ftable.destroy_cq(recv_cq);
        }
        if (send_cq) {
            ftable.destroy_cq(send_cq);
        }
        if (srq) {
            ftable.destroy_srq(srq);
        }
        if (pd) {
            ftable.dealloc_pd(pd);
        }
    }
    return status;
}
...
static int ibgda_create_dct(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_ep **ep_ptr,
    ...
    ib_qp_attr_ex.cap.max_send_wr = ibgda_state->common.options->QP_DEPTH;
    ib_qp_attr_ex.cap.max_recv_wr = ibgda_state->common.options->QP_DEPTH;
...
out:
    if (status) {
        if (ib_qp) {
            ...
            if (_status) {
                NVSHMEMI_ERROR_PRINT("ibv_destroy_qp for dct failed.\n");
            }
        }
        if (ep) {
            free(ep);
...
static int ibgda_validate_dct_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    int status = 0;
    int num_dct_eps = ibgda_state->common.options->IBGDA_NUM_DCT;
...
static int ibgda_populate_dct_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    for (int i = 0; i < num_dct_handles / n_devs_selected; ++i) {
        for (int j = 0; j < n_devs_selected; j++) {
            ...
            struct ibgda_device *device =
                (struct ibgda_device *)ibgda_state->common.devices + dev_idx;
...
static int ibgda_calculate_dci_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
                                         ...
                                         int mpc) {
    int status = 0;
    int num_dci_eps = ibgda_state->common.options->IBGDA_NUM_DCI;
    int num_shared_dci_eps = ibgda_state->common.options->IBGDA_NUM_SHARED_DCI;
...
static int ibgda_setup_dci_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    INFO(ibgda_state->common.log_level, "Creating %d DCI QPs (shared: %d, exclusive: %d)",
...
static int ibgda_populate_dci_gpu_data(nvshmemt_ibgda_state_t *ibgda_state,
                                       nvshmemi_ibgda_device_qp_t *dci_h,
    ...
    for (int i = 0; i < num_dci_handles / n_devs_selected; ++i) {
        for (int j = 0; j < n_devs_selected; j++) {
            ...
            TRACE(ibgda_state->common.log_level, "Populating DCI at qp_idx #%d, cq_idx #%d",
                  arr_idx, cq_idx);
...
static int ibgda_setup_dci_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, int *num_dci_handles,
                                     int *num_shared_dci_handles,
                                     nvshmemi_ibgda_device_qp_t **dci_h,
                                     nvshmemi_ibgda_device_qp_t **dci_d) {
    int status = 0;
    int n_devs_selected = ibgda_state->n_devs_selected;
...
static int ibgda_calculate_rc_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    int status = 0;
    int num_rc_eps_per_pe = ibgda_state->common.options->IBGDA_NUM_RC_PER_PE;
...
static int ibgda_allocate_rc_structures(nvshmem_transport_t t, struct ibgda_device *device,
    ...
    if (device->rc.eps == NULL) {
        ...
        size_t old_size = device->rc.num_eps_per_pe * n_pes;
        size_t new_size = old_size + num_rc_eps;
        struct ibgda_ep **eps =
            (struct ibgda_ep **)realloc(device->rc.eps, new_size * sizeof(*device->rc.eps));
        NVSHMEMI_NULL_ERROR_JMP(eps, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                "allocation of rc.eps failed.");
        device->rc.eps = eps;
        memset(&device->rc.eps[old_size], 0, num_rc_eps * sizeof(*device->rc.eps));
    }
...
static int ibgda_setup_rc_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    int global_rc_first_index = ibgda_state->cur_qp_index;
    ...
    INFO(ibgda_state->common.log_level, "Creating %d RC QPs", num_eps_per_pe);
    for (int i = 0; i < num_eps_per_pe; ++i) {
        for (int j = 0; j < n_pes; ++j) {
            ...
            /* mapped_i indexes this device's RC array; global_mapped_i indexes rc_h/rc_d. */
            ...
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "ibgda_create_rc failed on RC #%d.", global_mapped_i);

            status = ibgda_get_rc_handle(&local_rc_handles[local_mapped_i],
                                         device->rc.eps[mapped_i], device);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "ibgda_get_rc_handle failed on RC #%d.", global_mapped_i);
        }
    }
...
out:
    if (status) {
        for (int i = 0; i < num_rc_eps; ++i) {
            int ep_index = rc_first_index + i;
            if (device->rc.eps && device->rc.eps[ep_index]) {
                int destroy_status = ibgda_destroy_ep(device->rc.eps[ep_index], ibgda_state);
                if (destroy_status) {
                    NVSHMEMI_ERROR_PRINT("ibgda_destroy_ep failed while rolling back RC #%d.\n",
                                         ep_index);
                }
                device->rc.eps[ep_index] = NULL;
            }
        }
    }
    if (local_rc_handles) {
...
static int ibgda_populate_rc_gpu_data(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
    ...
    /* CQs for all DCIs are packed before CQs for any RCs. */
    for (int i = 0; i < n_devs_selected; i++) {
        int dev_idx = ibgda_state->selected_dev_ids[i];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->common.devices + dev_idx;
        first_rc_cq_index += device->dci.num_eps;
    }
    ...
    if (num_rc_handles > 0) {
        for (int i = 0; i < n_devs_selected; i++) {
            ...
            for (int j = 0; j < device->rc.num_eps_per_pe * n_pes; j++) {
                ...
                int my_cq_index = first_rc_cq_index + qp_index;
                ...
                TRACE(ibgda_state->common.log_level,
                      "Populating cq at cq_idx #%d qpn: %u qp_type: %u", my_cq_index,
                      cq_h[my_cq_index].qpn, cq_h[my_cq_index].qp_type);
...
static int ibgda_setup_rc_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
    ...
    *num_rc_handles = 0;
    ...
    INFO(ibgda_state->common.log_level, "num_rc_handles: %d", *num_rc_handles);
    /* Calculate RC buffer sizes end */

    /* Allocate RC host memory start */
    if (*num_rc_handles > 0) {
        if (*rc_h == NULL) {
            TRACE(ibgda_state->common.log_level, "Allocating RC host memory");
            *rc_h = (nvshmemi_ibgda_device_qp_t *)calloc(*num_rc_handles, sizeof(**rc_h));
        } else {
            TRACE(ibgda_state->common.log_level, "Reallocating RC host memory old: %p", *rc_h);
        ...
        for (int i = ibgda_state->last_num_rcs; i < *num_rc_handles; i++) {
            TRACE(ibgda_state->common.log_level, "Initializing RC at index #%d", i);
...
static int ibgda_setup_cq_gpu_state(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
                                    int *num_cq_handles, nvshmemi_ibgda_device_cq_t **cq_h,
...
static int ibgda_post_gpu_device_state(
    ...
    TRACE(ibgda_state->common.log_level,
          "Posting device state DCI_d: %p, RC_d: %p, DCT_d: %p, CQ_d: %p", dci_d, rc_d, dct_d,
          cq_d);
...
static int ibgda_destroy_ep(struct ibgda_ep *ep, nvshmemt_ibgda_state_t *ibgda_state) {
    ...
    if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_DCT) {
        if (ep->ib_qp) {
            ...
            if (status) {
                NVSHMEMI_ERROR_PRINT("ibv_destroy_qp failed.\n");
            }
        }
    } else {
...
static void ibgda_get_device_qp(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    if (dev_qp->qpn != NVSHMEMI_IBGDA_USCALAR_INVALID) {
        TRACE(ibgda_state->common.log_level,
              "Not re-populating RC at ep_index #%d, qp_idx #%d, qpn: %u", ep_idx, ep->user_index,
              dev_qp->qpn);
        return;
    }
    ...
    ibgda_get_device_qp_mvars(&dev_qp->mvars);
}
...
static int ibgda_setup_gpu_state(nvshmem_transport_t t) {
    ...
    status = ibgda_setup_dci_gpu_state(ibgda_state, &num_dci_handles, &num_shared_dci_handles,
    ...
    status = ibgda_setup_cq_gpu_state(ibgda_state, t, &num_cq_handles,
    ...
    if (num_rc_handles ==
        ibgda_state->common.options->IBGDA_NUM_RC_PER_PE * n_devs_selected * t->n_pes) {
    ...
    TRACE(ibgda_state->common.log_level, "Populated DCT GPU data");

    status = ibgda_populate_dci_gpu_data(
        ibgda_state, dci_h, dci_d, ibgda_state->device_state_cache->cq_h, cq_d, num_dci_handles);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "ibgda_populate_dci_gpu_data failed.");

    TRACE(ibgda_state->common.log_level, "Populated DCI GPU data");
    ...
    TRACE(ibgda_state->common.log_level, "Populated RC GPU data");
    ...
    TRACE(ibgda_state->common.log_level, "Copied DCT GPU data");
    ...
    TRACE(ibgda_state->common.log_level, "Copied DCI GPU data");
    ...
    TRACE(ibgda_state->common.log_level, "Copied RC GPU data");
    ...
    TRACE(ibgda_state->common.log_level, "Copied CQ GPU data");
...
out:
    if (status) {
        ...
        if (ibgda_state->device_state_cache->cq_h) {
            free(ibgda_state->device_state_cache->cq_h);
        }
        if (ibgda_state->device_state_cache->rc_h) {
...
static int ibgda_connect_global_setup(nvshmemt_ibgda_state_t *ibgda_state, int num_selected_devs,
    ...
    for (int i = 0; i < num_selected_devs; i++) {
        ...
        if (dev_id < 0 || dev_id >= ibgda_state->common.n_raw_devices) {
...
static int ibgda_connect_device_resources(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    status = ibgda_create_qp_shared_objects(device);
    if (status) {
        return status;
    }
...
static int ibgda_connect_device_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    status = ibgda_setup_rc_endpoints(ibgda_state, device, portid, t,
                                      ibgda_state->common.options->IBGDA_NUM_RC_PER_PE);
    if (status) {
        return status;
    }
...
static int ibgda_connect_rc_only(nvshmemt_ibgda_state_t *ibgda_state, nvshmem_transport_t t,
    ...
    if (ibgda_state->common.options->IBGDA_NUM_RC_PER_PE < 1) {
        status = NVSHMEMX_SUCCESS;  // No RC endpoints to create
        goto out;
    }
...
int nvshmemt_ibgda_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    int init_dev_cnt = 0;
    int n_pes = t->n_pes;
    std::vector<bool> initialized_devices(ibgda_state->common.n_raw_devices, false);
    for (int i = 0; i < ibgda_state->n_devs_selected; i++) {
        int dev_idx = ibgda_state->selected_dev_ids[i];
        struct ibgda_device *device = (struct ibgda_device *)ibgda_state->common.devices + dev_idx;
        ...
        status = ibgda_connect_device_resources(ibgda_state, device, portid, t);
        if (status) {
            return status;
        }
...
int nvshmemt_ibgda_finalize(nvshmem_transport_t transport) {
    ...
    device_finalized.resize(ibgda_state->common.n_raw_devices, false);
    ...
    if (ibgda_device_state_h) {
        if (ibgda_device_state_h->globalmem.dcts) {
            cudaFree(ibgda_device_state_h->globalmem.dcts);
        }
        if (ibgda_device_state_h->globalmem.dcis) {
            cudaFree(ibgda_device_state_h->globalmem.dcis);
        }
        if (ibgda_device_state_h->globalmem.cqs) {
            cudaFree(ibgda_device_state_h->globalmem.cqs);
        }
        if (ibgda_device_state_h->globalmem.rcs) {
            cudaFree(ibgda_device_state_h->globalmem.rcs);
        }
    ...
    for (int i = 0; i < ibgda_state->n_devs_selected; i++) {
        ...
        status = ibgda_destroy_qp_shared_objects(device);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibgda_destroy_qp_shared_objects failed.\n");

        status = ibgda_destroy_dct_shared_objects(device);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    for (int i = 0; i < ibgda_state->common.n_dev_ids; i++) {
        ...
        if (dev_id < 0 || dev_id >= ibgda_state->common.n_raw_devices || device_finalized[dev_id]) {
            continue;
        }
        ...
        if (device->common_device.pd) {
            ...
            if (status) {
                INFO(ibgda_state->common.log_level,
                     "ibv_dealloc_pd failed for device %d Err: %d:%s.\n", i, errno,
                     strerror(errno));
            }
        }
...
static int ibgda_check_nic_mapping_memtypes(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    if (try_gpumem) {
        ...
        if (!ibgda_state->common.options->IB_DISABLE_DMABUF && ibgda_state->cuda_support_dmabuf) {
    ...
    out_try_gpumem:
        if (mobject) {
            if (mobject->has_nic_mapping) {
                ibgda_mobject_nic_unmap(mobject);
            }
            ibgda_gpu_mem_free(mobject);
        }
        mobject = NULL;
...
out:
    if (mobject) {
        ibgda_unmap_and_free_qp_uar(mobject);
    }
    return status;
}
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    nvshmemt_ibgda_state_t *ibgda_state = NULL;
    struct ibgda_device *device;
    ...
    ibgda_state = new (std::nothrow) nvshmemt_ibgda_state_t{};
    ...
    ibgda_state->common.log_level = nvshmemt_common_get_log_level(options);
    ibgda_state->common.options = options;
    ibgda_state->device_state_cache = device_state_cache;
    ibgda_state->skip_cst = true;
    ibgda_state->support_half_av_seg = true;

    if (nvshmemt_ibv_ftable_init(&ibv_handle, &ftable, ibgda_state->common.log_level)) {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    NVSHMEMI_NULL_ERROR_JMP(ibgda_state->common.devices, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "get_device_list failed \n");
    ibgda_state->common.n_raw_devices = num_devices;
    ...
    INFO(ibgda_state->common.log_level, "NVSHMEM_IBGDA_NIC_HANDLER: %s\n",
...

    {
        ...
        for (int i = 0; i < ibgda_state->common.n_dev_ids; i++) {
            ...
            struct ibgda_device *ibgda_dev =
                &((struct ibgda_device *)ibgda_state->common.devices)[dev_idx];
            struct nvshmemt_ib_common_device *dev = &ibgda_dev->common_device;
            ...
            if (!device_checked[dev_idx]) {
                ...
                if (!nvshmemt_ib_common_query_mlx5_caps(dev->context)) {
                    NVSHMEMI_WARN_PRINT(
                        "device %s is not enumerated as an mlx5 device. Skipping...\n", name);
                ...
                if (nic_handler_request == IBGDA_NIC_HANDLER_CPU_GDRCOPY) {
                    ...
                    if (status) {
                        ...
                        ibgda_dev->nic_handler = IBGDA_NIC_HANDLER_GPU;
                    }
                }

                status = ibgda_check_nic_mapping_memtypes(ibgda_state, ibgda_dev,
                                                          nic_mapping_memtype_request);
                if (status) {
                    NVSHMEMI_WARN_PRINT(
                        "device %s cannot allocate buffer on the specified memory type. "
                        "Skipping...\n",
                        name);
                    ftable.close_device(dev->context);
                    if (dev->pd) {
                ...
                if (ibgda_dev->support_nic_buf_on_gpumem &&
                    !ibgda_state->common.options->IB_DISABLE_DMABUF &&
                    !ibgda_state->dmabuf_support_for_control_buffers) {
                    INFO(ibgda_state->common.log_level,
                         "The system does not support registering the NIC control buffers with "
                         "DMABUF. Fallback to use either nv_peer_mem or nvidia_peermem.\n");
                }
                ...
                status = nvshmemt_ib_common_query_endianness_conversion_size(
                    &atomic_host_endian_size, dev->context);
                NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                      "query_endianness_conversion_size failed\n");
            }
            ...
            ibgda_state->common.dev_ids[write_idx] = ibgda_state->common.dev_ids[i];
            ibgda_state->common.port_ids[write_idx] = ibgda_state->common.port_ids[i];
            write_idx++;
        }
        ibgda_state->common.n_dev_ids = write_idx;

        if (!ibgda_state->common.n_dev_ids) {
            INFO(ibgda_state->common.log_level,
                 "no suitable IB device found after filtering, exiting");
            status = NVSHMEMX_ERROR_INTERNAL;
            goto out;
        }
    }
    ...
    nvshmemt_ib_common_log_device_assignment(ibgda_state->common);

    status = nvshmemt_ib_common_discover_pci_paths(
        transport, ibgda_state->common, sizeof(struct ibgda_device), &ftable, &mlx5dv_ftable);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "PCI path discovery failed.\n");

    for (int i = 0; i < transport->n_devices; i++) {
        if (((struct ibgda_device *)ibgda_state->common.devices)[ibgda_state->common.dev_ids[i]]
                .common_device.data_direct &&
            !ibgda_state->common.options->IBGDA_NUM_RC_PER_PE_provided) {
            // Need 8 QPs for achieving bandwidth in data direct device
            ibgda_state->common.options->IBGDA_NUM_RC_PER_PE = 8;
            INFO(ibgda_state->common.log_level,
    ...
    if (nic_buf_on_gpumem) {
        ...
        INFO(ibgda_state->common.log_level, "NIC buffer will be on GPU memory.\n");
    } else {
        ibgda_nic_buf_location = IBGDA_MEM_TYPE_HOST;
    ...
    if (nic_handler == IBGDA_NIC_HANDLER_CPU_GDRCOPY) {
        ...
        INFO(ibgda_state->common.log_level, "NIC handler will be GPU.\n");
    }
...
out:
    if (status) {
        ...
        if (transport) {
            ...
            if (transport->state) {
                delete (nvshmemt_ibgda_state_t *)transport->state;
            }
            free(transport);
        }
    }
