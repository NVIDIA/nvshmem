...
bool libfabric_is_coherent_platform(struct nvshmemi_cuda_fn_table *table, int log_level) {
    ...
    r = CUPFN(table, cuDeviceGetAttribute(&attr, coherent_platform_attribute, dev));
    if (r == CUDA_ERROR_INVALID_VALUE) {
        INFO(log_level,
             "Coherent-platform probe: CUDA does not know attribute %d "
             "(old driver/runtime); treating as non-coherent.",
             static_cast<int>(coherent_platform_attribute));
        return false;
    }
    if (r != CUDA_SUCCESS) {
        INFO(log_level,
...
#define NVSHMEMT_LIBFABRIC_NZ_ERROR_JMP(status, err, label, ...)                             \
    do {                                                                                     \
        if (unlikely((status) != 0)) {                                                       \
            fprintf(stderr, "%s:%d: libfabric status: %d (%s) ", __FILE__, __LINE__, status, \
                    fi_strerror((status) < 0 ? -(status) : (status)));                       \
...
static int nvshmemt_libfabric_gdr_process_amo(nvshmem_transport_t transport,
                                              ...
                                              uint32_t sequence_count, uint8_t preceding_put_count);
...
nvshmemt_libfabric_imm_cq_data_hdr_t get_write_with_imm_hdr(uint64_t imm_data) {
    return static_cast<nvshmemt_libfabric_imm_cq_data_hdr_t>(static_cast<uint32_t>(imm_data) >>
                                                             imm_header_bit_shift);
}
...
static std::pair<bool, int> accumulate_signal(signal_seq_map &completion_map, uint32_t seq,
    ...
    nvshmemt_libfabric_comp_entry_t fresh_entry = nvshmemt_libfabric_signal_comp_entry{op, delta};

    auto insert_result = completion_map.insert(seq, fresh_entry);
    auto *signal_entry = std::get_if<nvshmemt_libfabric_signal_comp_entry>(insert_result.first);
...
static inline std::pair<nvshmemt_libfabric_signal_state_t *, std::unique_lock<std::recursive_mutex>>
...
int check_signal_delivery_status(nvshmemt_libfabric_state_t *state) {
    if (unlikely(state->signal_delivery_status.load(std::memory_order_acquire) != 0)) {
...
static inline int get_next_seq_num_with_retry(
    nvshmem_transport_t transport, nvshmemt_libfabric_endpoint_seq_counter_t &seq_counter,
    uint32_t *sequence_count, int qp_index, nvshmemt_libfabric_try_again_call_site_t call_site) {
...
static int nvshmemt_libfabric_gdr_complete_amos(nvshmem_transport_t transport) {
    ...
    status = fi_recv(ep.endpoint, (void *)done.op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                     fi_mr_desc(libfabric_state->mrs[ep.domain_index]), FI_ADDR_UNSPEC,
                     &done.op->ofi_context);
    NVSHMEMT_LIBFABRIC_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    if (done.is_fetch_amo) {
        ...
        do {
            status = fi_send(ep.endpoint, (void *)resp_op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                             fi_mr_desc(libfabric_state->mrs[ep.domain_index]), done.src_addr,
                             &resp_op->ofi_context);
        } while (try_again(transport, &status, &num_retries,
                           NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_PERFORM_GDRCOPY_AMO_SEND,
...

    {
        ...
        auto [signal_state_p, sig_lk] = get_signal_state_locked(libfabric_state, ep);
...
static inline void nvshmemt_libfabric_wake_signal_delivery_thread(
    ...
    if (state->signal_delivery_futex.exchange(1, std::memory_order_release) == 0) {
        syscall(SYS_futex, &state->signal_delivery_futex, FUTEX_WAKE_PRIVATE, 1, NULL, NULL, 0);
    }
}
...
int nvshmemt_libfabric_ack_aggregator_t::flush_peer(int pe, nvshmem_transport_t transport,
    ...
    status = gdrcopy_amo_ack(transport, ep, dest_addr, pending.range_end, pending.range_count,
...
int nvshmemt_libfabric_ack_aggregator_t::record_ack(int pe, uint16_t seq_num,
    ...
    if (!pending.has_range) {
        ...
        uint16_t next_expected = nvshmemt_libfabric_endpoint_seq_counter_t::seq_num_wrapup(
            (pending.range_end + 1) & nvshmemt_libfabric_endpoint_seq_counter_t::sequence_mask);
...
static void nvshmemt_libfabric_put_signal_ack_completion(
    nvshmemt_libfabric_state_t *state, nvshmemt_libfabric_endpoint_t &ep,
    const nvshmemt_libfabric_gdr_amo_ack_op_t &ack_op, fi_addr_t addr) {
    auto [signal_state_p, sig_lk] = get_signal_state_locked(state, ep);
    nvshmemt_libfabric_signal_state_t &signal_state = *signal_state_p;
    ...
    if (qstatus > 0) {
        if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            for (int j = 0; j < qstatus; j++) {
                status =
                    nvshmemt_libfabric_gdr_process_completion(transport, ep, entry[j], addr[j]);
                if (status) {
                    return NVSHMEMX_ERROR_INTERNAL;
                }
            }
        ...
        NVSHMEMI_WARN_PRINT(
            "Received a CQE with error. RC: %d. Error: %d (%s)", err_entry.err,
 ...
 */
/* Precondition: caller already holds host_ep_progress_lock. */
static inline int progress_host_eps_locked(nvshmem_transport_t transport) {
    nvshmemt_libfabric_state_t *state = (nvshmemt_libfabric_state_t *)transport->state;
    int status = 0;

    for (int i = 0; i < state->num_host_domains && !status; i++) {
...
static inline int try_progress_host_eps(nvshmem_transport_t transport) {
    nvshmemt_libfabric_state_t *state = (nvshmemt_libfabric_state_t *)transport->state;
    ...
    int status = progress_host_eps_locked(transport);
    state->host_ep_progress_lock.clear(std::memory_order_release);
    return status;
}
...
static int drain_completions(nvshmem_transport_t transport, int qp_index) {
    ...
    if (libfabric_state->use_auto_progress) {
        ...
        if (qp_index == NVSHMEMX_QP_HOST) {
            /* User-thread path: host_ep_progress_lock is already held by the caller. */
            status = progress_host_eps_locked(transport);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Error in host progress: %d.\n", status);
            goto out;
        }
        /* Proxy path: try-drain host EPs (skip if user is draining), then drain proxy EPs. */
        status = try_progress_host_eps(transport);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
static void *nvshmemt_libfabric_signal_delivery_thread(void *arg) {
    ...
    while (!state->signal_delivery_stop.load(std::memory_order_relaxed)) {
        if (!state->signal_work_queue.pop(work)) {
            ...
            if (!state->signal_work_queue.pop(work)) {
                syscall(SYS_futex, &state->signal_delivery_futex, FUTEX_WAIT_PRIVATE, 0, NULL, NULL,
                        0);
                continue;
            }
        }
        empty_spin_count = 0;
        int status = nvshmemt_libfabric_gdr_process_amo(
            transport, work.op, work.send_elems, work.sequence_count, work.preceding_put_count);
        if (unlikely(status)) {
            NVSHMEMI_WARN_PRINT("Failed call to nvshmemt_libfabric_gdr_process_amo: %d", status);
            state->signal_delivery_status.store(status, std::memory_order_release);
            state->signal_delivery_stop.store(1, std::memory_order_release);
            break;
        }
    }
    return NULL;
...
static int nvshmemt_libfabric_drain_deferred_work(nvshmem_transport_t transport) {
    ...
    while (libfabric_state->deferred_work_queue.pop(item)) {
        if (auto *work = std::get_if<signal_delivery_work_entry>(&item)) {
            ...
            host_ep_submit_guard host_guard(libfabric_state, *ack->ep);
            status =
                gdrcopy_amo_ack(transport, *ack->ep, ack->src_addr, ack->ack_payload.ack_seq_num,
                                ack->ack_payload.ack_count, ack->ack_payload.ack_num_ops);
        }
        if (unlikely(status)) {
            return status;
        }
...
static int nvshmemt_libfabric_progress(nvshmem_transport_t transport, int qp_index) {
    ...
    if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        status = check_signal_delivery_status(libfabric_state);
        if (unlikely(status)) {
            return status;
        }
         ...
         * -> gdrcopy_amo_ack uses the caller-held host_ep_progress_lock for host EPs. */
        if (libfabric_state->host_signal_state.ack_aggregator) {
            host_ep_submit_guard host_guard(libfabric_state, *libfabric_state->eps[0]);
            auto [signal_state_p, sig_lk] =
                get_signal_state_locked(libfabric_state, *libfabric_state->eps[0]);
            status =
                signal_state_p->ack_aggregator->flush_stale(transport, *libfabric_state->eps[0]);
            if (status) {
                return NVSHMEMX_ERROR_INTERNAL;
            }
        }
    ...
    for (int i = domain_start_idx; i < domain_end_idx; i++) {
        ...
        do {
            do {
                ...
                try_again(transport, &status, &num_retries,
                          NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDR_PROCESS_AMOS_GET_NEXT_NOT_ACK,
                          qp_index, progress_type::CompletionsOnly));
            num_retries = 0;
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "getNextAmoOps failed.\n");
...
static int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
    ...
    switch (kind) {
        case completion_kind::StandalonePutAckReq: {
            map_seq = static_cast<uint32_t>(entry.data) &
                      nvshmemt_libfabric_endpoint_seq_counter_t::bit_mask;
            uint8_t put_count = (static_cast<uint32_t>(entry.data) >>
                                 nvshmemt_libfabric_endpoint_seq_counter_t::bit_shift) &
                                0xFF;
            nvshmemt_libfabric_comp_entry_t ack_comp_entry =
                nvshmemt_libfabric_put_ack_entry{addr, ep.ep_index, put_count};
        ...
        case completion_kind::PutSignalSignal: {
            ...
            int progress_count = static_cast<int>(sig_op->num_writes);
            ...
            const auto [drainable, signal_status] =
                accumulate_signal(*completion_map, map_seq, op, progress_count);
            status = signal_status;
            if (status) {
                goto out;
            }
...

    {
        ...
        while (true) {
            ...
            if (auto *signal_entry = std::get_if<nvshmemt_libfabric_signal_comp_entry>(it)) {
...
static int nvshmemt_libfabric_quiet(struct nvshmem_transport *tcurr, int /*pe*/, int qp_index) {
    ...
    for (;;) {
         ...
         * progress_host_eps_locked (via progress -> drain_completions) will not
         * re-acquire either. Lock order: host_ep_progress_lock -> signal_state.mtx. */
        host_ep_submit_guard host_guard(libfabric_state, *libfabric_state->eps[ep_start_idx]);
        ...
        uint64_t total_completed = 0;
        {
            ...
            if (signal_state.ack_aggregator) {
                ...
                status =
                    signal_state.ack_aggregator->flush_all(tcurr, *libfabric_state->eps[ep_idx]);
                if (unlikely(status)) {
                    break;
                }
            }
...
static int nvshmemt_libfabric_rma_impl(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
    ...
    bool put_more_follows = verb.desc == NVSHMEMI_OP_PUT && attrs &&
        ...
        if (put_more_follows) {
            ...
            do {
                status = fi_writemsg(ep.endpoint, &p_op_msg, msg_flags);
                               ...
                               NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_RMA_IMPL_OP_PUT, ep.qp_index,
                               progress_type::All));
            libfabric_state->pending_batch_ep[slot_impl] = status ? -1 : ep_idx;
        } else {
            do {
...
out:
    ...
    if (unlikely(status)) {
        ep.submitted_ops--;
        NVSHMEMI_ERROR_PRINT("Received an error when trying to post an RMA operation: %d (%s).\n",
                             status, fi_strerror(status < 0 ? -status : status));
    }
...
static int nvshmemt_libfabric_rma_scalar(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
                                         rma_memdesc_t *remote, rma_memdesc_t *local,
                                         rma_bytesdesc_t bytesdesc, int qp_index,
                                         const nvshmem_transport_op_attrs_t *attrs) {
    nvshmemt_libfabric_state_t *libfabric_state = get_libfabric_state(tcurr);
    ...
    int status = 0;
    /*
     * Use the same EP while FI_MORE doorbell is deferred (RMA only). Slot by caller role so host
     * and proxy do not interfere.
     */
    ...
    if constexpr (with_attrs) {
        nvshmem_transport_op_attrs_t scalar_attrs;
        if (attrs && libfabric_state->disable_implicit_batch_rma) {
            scalar_attrs = *attrs;
...
static int nvshmemt_libfabric_rma_with_hints(struct nvshmem_transport *tcurr, int pe,
                                             ...
                                             int qp_index,
                                             const nvshmem_transport_op_attrs_t *attrs) {
    if (unlikely(attrs && (attrs->hints & NVSHMEMI_REGION_HINT_BATCH_RMA) != 0 &&
                 attrs->issuer_id && attrs->region_id)) {
        bool handled;
    ...
    return nvshmemt_libfabric_rma_scalar<true>(tcurr, pe, verb, remote, local, bytesdesc, qp_index,
                                               attrs);
}
...
static int nvshmemt_libfabric_rma(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
                                  rma_memdesc_t *remote, rma_memdesc_t *local,
                                  rma_bytesdesc_t bytesdesc, int qp_index) {
    return nvshmemt_libfabric_rma_scalar<false>(tcurr, pe, verb, remote, local, bytesdesc, qp_index,
                                                nullptr);
}
...
static int nvshmemt_libfabric_gdr_amo(struct nvshmem_transport *transport, int pe, void *curetptr,
    ...
    if (is_signal_only_op(verb.desc)) {
        ...
        status = get_next_seq_num_with_retry(
            transport, seq_counter, &sequence_count, qp_index,
            NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDR_AMO_GET_NEXT_SENDS);
        if (status) {
            goto out;
        }
        ...
        status = nvshmemt_libfabric_gdr_signal(transport, pe, curetptr, verb, remote, bytesdesc,
                                               qp_index, sequence_count, 0, ep, ppc);
    ...
    amo->send_amo.op = (nvshmemi_amo_t)(verb.desc | (verb.is_float ? NVSHMEMI_AMO_FLOAT_BIT : 0));
...
static int nvshmemt_libfabric_amo(struct nvshmem_transport *transport, int pe, void * /*curetptr*/,
    ...
    if (status) {
        goto out;  // Status set by try_again
    }
...
static int nvshmemt_libfabric_gdr_signal(struct nvshmem_transport *transport, int pe,
    ...
    signal->op = static_cast<uint8_t>(verb.desc | (verb.is_float ? NVSHMEMI_AMO_FLOAT_BIT : 0));
    ...
    /* Piggyback pending ACK for this destination PE if available */
    {
        ...
        if (signal_state.ack_aggregator &&
            signal_state.ack_aggregator->try_extract_for_peer(pe, ack_range_end, ack_range_count,
                                                              ack_signal_count)) {
...
static int nvshmemt_libfabric_put_signal_unordered(struct nvshmem_transport *tcurr, int pe,
...

    {
        ...
        status = get_next_seq_num_with_retry(
            tcurr, seq_counter, &sequence_count, qp_index,
            NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_PUT_SIGNAL_UNORDERED_SEQ);
        if (unlikely(status)) {
           ...
           write_local.size() == write_bytes_desc.size());
    {
        for (size_t i = 0; i < write_remote.size(); i++) {
            status = nvshmemt_libfabric_rma_impl(tcurr, pe, write_verb, &write_remote[i],
                                                 &write_local[i], write_bytes_desc[i], qp_index,
                                                 nullptr, &sequence_count, ep);
            if (unlikely(status)) {
                NVSHMEMI_ERROR_PRINT(
        ...
        status = nvshmemt_libfabric_gdr_signal(tcurr, pe, NULL, sig_verb, sig_target,
                                               sig_bytes_desc, qp_index, sequence_count,
                                               (uint16_t)write_remote.size(), ep, ppc);
    }
out:
    if (status) {
        NVSHMEMI_ERROR_PRINT(
...
skip:
    ...
    for (int ep_idx = ep_idx_start; ep_idx < ep_idx_end; ep_idx++) {
        ...
        do {
            ...
            if (libfabric_state->prov_infos[domain_idx]->caps & FI_FENCE) {
                flags |= FI_FENCE;
            }
...
static int nvshmemt_libfabric_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    /* Initialize state-level signal ordering state */
    {
        ...
        if ((uint32_t)npes > base_hwm) {
            NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                               "tx_size=%u too small for npes=%d (base_hwm=%u < npes): aggregate "
                               "ack pressure cannot be bounded below tx ring depth. Use a NIC with "
                               "deeper tx ring, or enable the long-term try_again fix.\n",
        ...
        uint8_t put_ack_freq = static_cast<uint8_t>(std::min<uint32_t>(
            nvshmemt_libfabric_endpoint_seq_counter_t::PUT_ACK_FREQ_CAP, put_ack_freq_u));
        ...
        state->host_signal_state.ack_aggregator =
            std::make_unique<nvshmemt_libfabric_ack_aggregator_t>(npes);
        ...
        state->proxy_signal_state.ack_aggregator =
            std::make_unique<nvshmemt_libfabric_ack_aggregator_t>(npes);
    ...
    for (size_t i = 0; i < state->prov_infos.size(); i++) {
        ...
        if (state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            ...
            state->recv_buf.push_back(
                std::vector<nvshmemt_libfabric_gdr_op_ctx_t>(num_sends + num_recvs));
            ...
            state->op_queue.emplace_back(std::unique_ptr<threadSafeOpQueue>(
                new threadSafeOpQueue(!state->use_auto_progress)));
    ...
    if (state->use_staged_atomics) {
        ...
        state->signal_delivery_status.store(0, std::memory_order_relaxed);
...
static int nvshmemt_libfabric_finalize(nvshmem_transport_t transport) {
    ...
    if (libfabric_state->use_staged_atomics && libfabric_state->signal_delivery_transport) {
        ...
        syscall(SYS_futex, &libfabric_state->signal_delivery_futex, FUTEX_WAKE_PRIVATE, 1, NULL,
                NULL, 0);
